...
func main() {
	...
	NewPerson := PeopleFactory()
	// pslice := make(PersonSlice, 5)
	pslice := PersonSlice{
		NewPerson("a", "a"),
		NewPerson("b", "c"),
		NewPerson("a", "b"),
...
func PeopleFactory() func(first, last string) *Person {
	peopleCreated := 0
	return func(first, last string) *Person {
		peopleCreated++
		return &Person{peopleCreated, first, last}
	}
}
